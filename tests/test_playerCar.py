...
class TestSinglePlayerGame:
    ...
    @pytest.fixture
    def mock_game(self):
        """Isolated game instance with mocked hardware components (video, audio, assets)."""
    ...
    def test_player_movement_logic(self, mock_game):
        """Verification of player positioning and direction during left/right movement."""
        player = mock_game.player

        # Test Move Left
        initial_x = player.rect.x
        player.update({player.left_key: True, player.right_key: False}, 0.1)
        assert player.rect.x < initial_x
        assert player.direction == -1

        # Test Move Right
        player.update({player.left_key: False, player.right_key: True}, 0.1)
        assert player.direction == 1

    def test_player_boundary_constraints(self, mock_game):
        """Ensures the player car remains within the horizontal window limits."""
        player = mock_game.player
